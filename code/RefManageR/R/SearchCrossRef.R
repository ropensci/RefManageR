...
SearchCrossRef <- function(query, max.results = 5, sort='relevance', year=NULL, min.relevance=80,
                           temp.file='tfile.bib', delete.file=TRUE){
  results <- getForm("http://search.labs.crossref.org/dois", q=query, year=year, sort=sort,  
                     rows=max.results)
  
  fromj <- RJSONIO::fromJSON(results)
 # browser()
  num.res <- min(max.results, length(fromj))
