...
GSImport <- function(scholar.id, start = 0, num.results = 100, sort.by.date = FALSE,
                     encoding = 'UTF-8', bib.violation = .BibOptions$bib.violation){
  num.results <- min(num.results, 100)
  ps <- ifelse(num.results <= 20, 20, 100)
  ...
  cites <- cites[1:min(num.results, length(cites))]
