...
GSSearch <- function(title, authors='', journal='', low.year='', high.year='', exact=FALSE,
                     sort.by.date = FALSE, encoding = 'UTF-8'){
  ...
  argh <- list(uri="http://scholar.google.com/scholar", as_q='', as_authors=authors, as_publication=journal, as_ylo=low.year, 
               as_yhi=high.year, hl='en', btnG='Search', oe = encoding, .opts=list(verbose=FALSE))
  ...
  doc <- do.call(getForm, argh)
  browser()
  cites <- xpathApply(htmlParse(doc, encoding = encoding), 'div[@class="gs_r"]')
  dd = htmlParse(z)
