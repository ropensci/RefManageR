...
library("knitr")
options(prompt = "R> ")
opts_chunk$set(comment = NA, prompt = TRUE, highlight = FALSE)
# library(lubridate)
...
knit_hooks$set(message = function(x, options) {
    options$comment = NA
    return(paste0('\\begin{lstlisting}[style=output]\n',
      x,
    '\\end{lstlisting}'))
})
...
#     x <- gsub("[{]", "<{<", x)
#     x <- gsub("[}]", "<}<", x)
#     return(paste0('\\begin{lstlisting}[style=source]\n',
#       x,
#     '\\end{lstlisting}'))
# })
...
knit_hooks$set(output = newhook)
ref <- BibEntry("thesis", key = "scheiplthesis", date = "2011-03-17", url = 
...
bib[seq_len(3)]
bib[seq_len(3)] <- list(c(date="2013-12"),
