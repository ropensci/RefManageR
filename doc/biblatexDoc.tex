...
\definecolor{shadecolor}{rgb}{0.973, 0.973, 0.973}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{bib} \hlkwb{<-} \hlkwd{as.BibEntry}\hlstd{(}\hlkwd{c}\hlstd{(}\hlkwc{key} \hlstd{=} \hlstr{"carroll2014dietary"}\hlstd{,} \hlkwc{bibtype} \hlstd{=} \hlstr{"article"}\hlstd{,} \hlkwc{year} \hlstd{=} \hlnum{2014}\hlstd{,}
  \hlkwc{title} \hlstd{=} \hlstr{"Estimating the distribution of dietary consumption patterns"}\hlstd{,} \hlkwc{author} \hlstd{=}
    \hlstr{"Raymond J. Carroll"}\hlstd{,} \hlkwc{journal} \hlstd{=} \hlstr{"Statistical Science"}\hlstd{,} \hlkwc{pubstate} \hlstd{=} \hlstr{"inpress"}\hlstd{))}
\hlstd{bib}
\end{alltt}
\begin{verbatim}
...
The default values of all options can be restored using \code{BibOptions(restore.defaults = TRUE)}.
...
\section[Using RefManageR for Citations]{Using \ourpkg{} for HTML and Markdown Citations and Bibliographies}\label{sec_cite}
