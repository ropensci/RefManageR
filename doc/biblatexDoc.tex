...
\hlkwd{c}\hlstd{(}\hlkwc{old} \hlstd{= old.opt.val[[}\hlnum{1L}\hlstd{]],} \hlkwc{new} \hlstd{=} \hlkwd{BibOptions}\hlstd{()}\hlopt{$}\hlstd{check.entries)}
\end{alltt}
\begin{verbatim}
##     old     new 
## "error" "FALSE"
...
\end{verbatim}
\begin{alltt}
\hlkwd{BibOptions}\hlstd{(old.opt.val)}  \hlcom{# restore the old value of the options}
\end{alltt}
\end{kframe}
\end{knitrout}
...
Finally, the \code{GetPubMedByID} function uses Entrez's Efetch to obtain bibliography data given a vector of PubMed ID's.  We can use the PubMed ID we just obtained to get the \Bibtex{} entry from Entrez and compare it with the one already in our bibliography from Google Scholar.
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.973, 0.973, 0.973}\color{fgcolor}\begin{kframe}
\begin{alltt}
...
\end{alltt}
\begin{lstlisting}
## ignoring entry 'schieplthesis' (line 1) because :
