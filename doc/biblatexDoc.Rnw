...
<<ReadZotero, tidy=FALSE, highlight=TRUE, echo=-1>>=
knit_theme$set('fine_blue')
...
<<ReadGS1, tidy=FALSE, highlight=TRUE>>=
...
<<ReadGS2, tidy=FALSE, highlight=TRUE>>=
## RJC's Google Scholar profile is at: 
## http://scholar.google.com/citations?user=CJOHNoQAAAAJ
rjc.bib <- ReadGS(scholar.id = 'CJOHNoQAAAAJ', sort.by.date = FALSE, 
                  limit = 3)
rjc.bib
rbind(rjc.bib['cites'], rjc.bib['bibtype'])
@
...
<<ReadGS3, tidy=FALSE, highlight=TRUE>>=
## RJC's Google Scholar profile is at: 
## http://scholar.google.com/citations?user=CJOHNoQAAAAJ
rjc.bib <- ReadGS(scholar.id = 'CJOHNoQAAAAJ', sort.by.date = FALSE, 
                  limit = 10, bib.violation = 'error')
rjc.bib2 <- ReadGS(scholar.id = 'CJOHNoQAAAAJ', sort.by.date = FALSE, 
                  limit = 10, bib.violation = 'warn')
length(rjc.bib) == length(rjc.bib2)
## the offending entry.  RJC missing because author list too long!!!
rjc.bib2[[7]]
@
\subsection{CrossRef}
...
<<ReadCR1, tidy=TRUE, highlight=TRUE>>=
...
The list extraction operator, \code{'[['}, is used for extacting a single bibentry object by position (an integer) or the entry key (a string).  List assignment, \code{'[[<-'} is used for replacing one entry in a BibEntry object with another.
\subsection{Merging}
