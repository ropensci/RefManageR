...
<<Poppler1, cache = FALSE, warning=FALSE, message=FALSE, tidy = FALSE, eval = FALSE>>=
tmpdir <- tempdir()
...
<<toBibtex, cache = TRUE, tidy = FALSE, echo = -1>>=
knit_hooks$set(output = newhook)
...
<<open, eval = FALSE>>=
open(pkg.bib[["base"]])  # will use use the 'url' field
...
bib <- ReadBib(file.name)
@
\code{Citet(bib, "loh")} produces \Sexpr{Citet(bib, "loh")}, a "textual" citation using an entry key.  It is possible to cite in parentheses by `year' using \code{Citep(bib, year = "1899", .opts = list(cite.style = "alphabetic"))} \Sexpr{Citep(bib, year = "1899", .opts = list(cite.style = "alphabetic"))}.  Next, three works by Averroes are cited \code{AutoCite(bib, author = "averroes", .opts = list(super = TRUE, cite.style = "numeric"))} $\Sexpr{AutoCite(bib, author = "averroes", .opts = list(super = TRUE, cite.style = "numeric"))}$.  There is some support for resolving ambiguous citations; consider \code{Citet(bib, author = "Baez")} \Sexpr{Citet(bib, author = "Baez")}.  Finally, the bibliography is printed using \code{PrintBibliography}, though note that we should really have stuck with just one value for \code{cite.style}.
<<CitePrint, cache = TRUE>>=
