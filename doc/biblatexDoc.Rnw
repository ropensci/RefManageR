...
bib <- as.BibEntry(c(key = "carroll2014dietary", bibtype = "article", year = 2014,
  title = "Estimating the distribution of dietary consumption patterns", author = 
    "Raymond J. Carroll", journal = "Statistical Science", pubstate = "inpress"))
bib
@
...
<<asBibEntry2, cache=TRUE, tidy = FALSE, warning=FALSE, message=FALSE>>=
...
<<asBibEntry3, cache=TRUE, tidy = FALSE, warning=FALSE, message=FALSE>>=
pkg.bib <- RefManageR:::MakeCitationList(pkg.bib)
pkg.bib["boot"]
pkg.bib[key = "boot"]
@
Using \code{pkg.bib["boot"]} matches the entry with key exactly ``boot''.  Using \code{pkg.bib[key = "boot"]}, we match any entry whose key contains the string ``boot''.
