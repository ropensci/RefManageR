...
<<ReadCR1, tidy=TRUE, highlight=TRUE, cache=TRUE>>=
ReadCrossRef(query = 'rj carroll measurement error', limit = 3, sort = "relevance", 
             min.relevance = 80, verbose = FALSE)
@
...
<<Search1, tidy=TRUE, highlight=TRUE, echo=-1>>=
setwd("C:/Users/Matthew/Documents/biblatex/RefManageR/code/RefManageR/sampleData")
bib <- ReadBib('biblatexExamples.bib', check = FALSE)
...
List assignment, \code{'[[<-'} is used for replacing one entry in a BibEntry object with another.  The below example uses a bibliography of just under 500 works of Raymond J.\ Carroll indexed on Google Scholar.
<<RJC1, tidy=TRUE, highlight=TRUE, cache=TRUE>>=
# Some selected works of Raymond J. Carroll
...
ind <- SearchBib(bib, author = "!Carroll", return.index = TRUE)
bib[ind]$author  
@
...
<<RJC2, tidy=TRUE, dependson=RJC1, highlight=TRUE, cache=TRUE>>=
bib <- bib[-ind[4L]]
