...
py.bibtex.parser <- NULL
#' @importFrom reticulate import py_module_available
#' @noRd
.onLoad <- function(libname, pkgname) {
  if (!reticulate::py_module_available("bibtexparser"))
    install_bibtexparser()
  py.bibtex.parser <<- reticulate::import("bibtexparser", delay_load = TRUE,
                                          convert = TRUE)
}


install_bibtexparser <- function(method = "auto", conda = "auto") {
  on.windows <- Sys.info()[["sysname"]] == "Windows"
  reticulate::py_install("bibtexparser", method = method, conda = conda,
                         pip = TRUE)
}
